...
// User1
users.user1.totalTasks = data.reduce((sum, task) => {
  if (task.userId == 1) {
    sum++;
  }
  return sum;
...
users.user1.totalIncomplete = data.reduce((sum, task) => {
  if (task.userId == 1 && task.completed == false) {
    sum++;
  }
  return sum;
}, 0);
...
// User3
users.user3.totalTasks = data.reduce((sum, tasks) => {
  if (tasks.userId == 3) {
    sum++;
  }
  return sum;
...
users.user3.totalIncomplete = data.reduce((sum, task) => {
  if (task.userId == 3 && task.completed == false) {
    sum++;
  }
  return sum;
}, 0);
...
data.forEach(task => {
  if (task.userId == 3 && task.completed == false) {
    users.user3.incompleteTasks.push(task.title);
  }
});
// -------
